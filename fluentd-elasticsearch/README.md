...
Providing one's own version of [the images automatically built from this repository](https://github.com/xxlaila/kubernetes-yaml) will not be supported. This is an *optional* step. One has been warned.
...
```shell
kubectl create -f es-discovery-svc.yaml -n kube-dev
kubectl create -f es-svc.yaml -n kube-dev
kubectl create -f es-master.yaml -n kube-dev
kubectl rollout status -f es-master.yaml -n kube-dev

kubectl create -f es-ingest-svc.yaml -n kube-dev
kubectl create -f es-ingest.yaml -n kube-dev
kubectl rollout status -f es-ingest.yaml -n kube-dev

kubectl create -f es-data.yaml -n kube-dev
kubectl rollout status -f es-data.yaml -n kube-dev
```

Let's check if everything is working properly:

```shell
kubectl get svc,deployment,pods -l component=elasticsearch -n kube-dev
NAME                              TYPE           CLUSTER-IP       EXTERNAL-IP   PORT(S)        AGE
service/elasticsearch             ClusterIP      10.254.202.62    <none>        9200/TCP       103m
service/elasticsearch-discovery   ClusterIP      None             <none>        9300/TCP       103m
service/elasticsearch-ingest      ClusterIP      10.254.242.53    <none>        9200/TCP       96m
service/hq                        LoadBalancer   10.254.174.236   <pending>     80:32320/TCP   79m
service/kibana                    LoadBalancer   10.254.156.13    <pending>     80:49092/TCP   80m

NAME                              READY   UP-TO-DATE   AVAILABLE   AGE
deployment.extensions/es-data     2/2     2            2           88m
deployment.extensions/es-hq       1/1     1            1           79m
deployment.extensions/es-ingest   2/2     2            2           91m
deployment.extensions/es-kibana   1/1     1            1           80m
deployment.extensions/es-master   3/3     3            3           97m

NAME                             READY   STATUS    RESTARTS   AGE
pod/es-data-79c7b4bc8f-xrq5l     1/1     Running   0          88m
pod/es-data-79c7b4bc8f-zbqhb     1/1     Running   0          88m
pod/es-hq-596549c95d-gnjzd       1/1     Running   0          79m
pod/es-ingest-67c5467dc7-42d6b   1/1     Running   0          91m
pod/es-ingest-67c5467dc7-dbwpn   1/1     Running   0          91m
pod/es-kibana-5799c7d8cf-fhrd4   1/1     Running   0          80m
pod/es-master-7889fb58d4-29xrx   1/1     Running   0          97m
pod/es-master-7889fb58d4-2csw4   1/1     Running   0          97m
pod/es-master-7889fb58d4-wzggm   1/1     Running   0          97m
```
...
NAME            TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)    AGE
elasticsearch   ClusterIP   10.254.202.62   <none>        9200/TCP   3m
```

From any host on the Kubernetes cluster (that's running `kube-proxy` or similar), run:

```shell
curl http://10.254.202.62:9200
```
...
```shell
curl http://10.254.202.62:9200/_cluster/health?pretty
```
...
**Note:** The X-Pack plugin does not currently work with the `quay.io/xxlaila/kubernetes-yaml` image. See Issue #102
